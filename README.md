...
- **Side-by-side web compare** -- compare multiple envs via `env-name.localhost:port` hostname routing (Chrome/Firefox). HTTP-only — works for web UIs in the browser; non-HTTP ports (databases, gRPC) forward to the primary env only.
- **Additive compare** -- right-click "Web Compare" while forwarding; add/remove envs one at a time without stopping
...
| `Cloudev: Web Compare` | Add an env to compare web UI alongside the active forwarding target |
| `Cloudev: Remove Web Compare` | Remove an env from the compare session |
| `Cloudev: Open Web Compare` | Open all compared envs side by side in browser |
